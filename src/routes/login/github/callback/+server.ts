...
export async function GET(event: RequestEvent): Promise<Response> {
  ...
  try {
    ...
    if (existingUser) {
      const session = await lucia.createSession(existingUser.id, { attributes: { avatar: githubUser.avatar_url, username: githubUser.login } }, { sessionId: v4() });
      ...
      const session = await lucia.createSession(userId, { attributes: { avatar: githubUser.avatar_url, username: githubUser.login } }, { sessionId: v4() });
      const sessionCookie = lucia.createSessionCookie(session.id);
      event.cookies.set(sessionCookie.name, sessionCookie.value, {
        path: '.',
        ...sessionCookie.attributes,
