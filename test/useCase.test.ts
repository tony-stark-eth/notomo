...
describe('useCase', () => {
  ...
  describe('createNote', () => {
    it('should create a new note', async () => {
      const note = new Note(v4(), 'Test Note', 'This is a test note', ContentType.Text);
  ...
  describe('getNotes', () => {
    it('should retrieve all notes', async () => {
      const notes = await getNotes(ORM.em.fork(), v4(), 10, 0);
      expect(notes).toBeDefined();
      expect(notes).toBeInstanceOf(Array);
    });

    it('should retrieve notes with pagination', async () => {
      const notes = await getNotes(ORM.em.fork(), v4(), 5, 5);
      expect(notes).toBeDefined();
      expect(notes).toBeInstanceOf(Array);
